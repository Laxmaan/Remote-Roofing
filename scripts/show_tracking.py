...
def main(videoname, data_dir, track_log_dir):
  ...
  pth = osp.join(data_dir, videoname, 'img')
  ext = osp.splitext(glob.glob(pth+"/*")[0])[1]
  ...
  def redraw_fn(ind, axes):
    ...
    org_img = imread(osp.join(data_dir, videoname, 'img', f'{ind+1:04d}{ext}'))
    ...
    if not redraw_fn.initialized:
      ...
      redraw_fn.bb3 = create_bbox(te_bbox, color='blue')
